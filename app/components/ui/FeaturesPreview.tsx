...
export default function FeaturesPreview({ className }: { className?: string }) {
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden rounded-3xl bg-[var(--color-brand)] p-5 flex flex-col gap-3.5${className ? ` ${className}` : ""}`}
    >
        ...
        <div className="mt-3 flex-1 flex flex-col">
          ...
          <div className="flex flex-col gap-2 flex-1">
            {current.results.map((result, i) => (
              <ResultCard key={result.name} result={result} relevant={i === 0} />
            ))}
          </div>
        </div>
      </div>
